...
from physical_gate.core import PROFILE, digest, keys
...
def setup_stack():
    ...
    authority=sign_authority(
        physical_authority_key,principal="human:owner",agent=sim.envelope["agent"],
        device=sim.envelope["device"],operations=PROFILE["operations"],
        action_digest=digest(sim.envelope["action"]),issued_ms=NOW-10,expires_ms=NOW+1000,
        nonce=sim.envelope["nonce"],profile_digest=sim.envelope["profile_digest"])
    ...
    signed_decision=issue_secure(gate,sim.envelope,sim.snapshot(),decision_key,NOW,
                                 authority_proof=authority,state_proof=state)
    assert signed_decision["payload"]["disposition"]=="ALLOW"
    decision_trust={signed_decision["key_id"]:decision_pub}
    ...
    return {
        "sim":sim,"execution_key":execution_key,"execution_pub":execution_pub,
        "authority":authority,"authority_trust":authority_trust,
        "state":state,"state_trust":state_trust,
        "signed_decision":signed_decision,"decision_trust":decision_trust,
        "dkeys":dkeys,"trust":trust,
    }


def decision_receipt(x):
    return build_decision_receipt(
        envelope=x["sim"].envelope,snapshot=x["sim"].snapshot(),
        physical_decision_signed=x["signed_decision"],physical_decision_trust=x["decision_trust"],
...
def test_canonical_scope_is_reference_verifier_compatible_and_exact_action_is_bound():
    x=setup_stack(); env=x["sim"].envelope
...
def test_simulated_execution_binds_as_canonical_ddcar_without_claiming_real_hardware():
    x=setup_stack(); env=x["sim"].envelope; receipt=decision_receipt(x)
    with tempfile.TemporaryDirectory() as tmp:
        executor=SecureExecutor(
            x["sim"],Ledger(os.path.join(tmp,"ledger.db")),x["decision_trust"],x["execution_key"],
            authority_trust=x["authority_trust"],state_trust=x["state_trust"])
        physical_execution=executor.dispatch_secure(
            env,x["signed_decision"],authority_proof=x["authority"],state_proof=x["state"])
        final=bind_simulated_execution(
            receipt,envelope=env,physical_decision_signed=x["signed_decision"],
            physical_decision_trust=x["decision_trust"],
            physical_execution_signed=physical_execution,
    ...
    assert verify_receipt(final,trust=x["trust"],mode="historical",
...
def test_untrusted_physical_execution_is_rejected_before_ddcar_binding():
    ...
    fake={"payload":{"status":"SIMULATED","device":env["device"],"nonce":env["nonce"],
                     "action_digest":digest(env["action"]),"decision_digest":"bad","completed_ms":NOW+1},
    ...
    with pytest.raises(ValueError,match="untrusted physical execution claim"):
        bind_simulated_execution(
            receipt,envelope=env,physical_decision_signed=x["signed_decision"],
            physical_decision_trust=x["decision_trust"],physical_execution_signed=fake,
...
def test_tampered_physical_authority_is_rejected_before_ddcar_sealing():
    x=setup_stack()
    bad=dict(x["authority"]); bad["payload"]=dict(bad["payload"],operations=[])
    with pytest.raises(ValueError,match="physical decision authority proof mismatch"):
...
def test_revoked_physical_authority_blocks_ddcar_construction():
    x=setup_stack()
    revoked={digest(x["authority"]["payload"])}
    with pytest.raises(ValueError,match="physical authority verification failed: AUTHORITY_REVOKED"):
        build_decision_receipt(
            envelope=x["sim"].envelope,snapshot=x["sim"].snapshot(),
            physical_decision_signed=x["signed_decision"],physical_decision_trust=x["decision_trust"],
            physical_authority_signed=x["authority"],state_signed=x["state"],
            physical_authority_trust=x["authority_trust"],state_trust=x["state_trust"],
            physical_authority_revocations=revoked,
            ddcar_trust=x["trust"],ddcar_authority_private=x["dkeys"]["authority"][0],
            authority_key_id="authority-key",ddcar_decision_private=x["dkeys"]["decision"][0],
            decision_key_id="decision-key",profile=PROFILE)
